       ...
       // et_amount.setText(amount.toString())
        ...
        btn_sendCash.setOnClickListener {
            ...
            if (et_amount.text.toString().isEmpty()){
                Toast.makeText(requireContext(),"Enter some amount for $receiverName", Toast.LENGTH_LONG).show()
                return@setOnClickListener
                }

            val amount = et_amount.text.toString().toLong()
            val action = SendCachFragmentDirections.actionSendCachFragmentToConfirmDialogFragment(receiverName,amount)
